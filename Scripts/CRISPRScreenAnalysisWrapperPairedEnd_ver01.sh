...
while getopts 'o:s:b:q:f:l:d:n:y' OPTION; do
  case "$OPTION" in
...
# transfer paired read R2
if [ -n "$Object_store_paths" ]; then
  Object_store_paths2=$(echo $Object_store_paths|sed "s|R1|R2|g")
  for path in $Object_store_paths2; do
      bsenme=$(basename -s ".gz" $path)
      bsenme="$fastq_directory""/""$bsenme"        
      o3-do.rb $path "gunzip -c {1} > $bsenme"
  done
...
################################################################################
# only run fastqc if -y option is set
################################################################################

if [ -z "$RunFastQC" ]; then
  echo "-y option not set, so not running fastqc."
  else
...
if [ -n "$sgRNA_library_file" ]; then
  # prepend all filenames with scratch directory
    ...
    allFastQFiles2=$(echo $allFastQFiles|sed "s|R1|R2|g")
  echo "Running mageck count."
  mageck count \
  -l $sgRNA_library_file \
  --fastq $allFastQFiles \
  --fastq-2 $allFastQFiles2 \
